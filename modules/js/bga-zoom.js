...
var ZoomManager = /** @class */ (function () {
  ...
  ZoomManager.prototype.zoomOrDimensionChanged = function () {
    const targetElement = this.settings.element;
    const wrapper = this.wrapper;
    const currentZoom = this._zoom;

    // --- Width ---
    const expectedWidth = wrapper.offsetWidth / currentZoom;
    const currentStyledWidth = parseFloat(targetElement.style.width);
    if (
      isNaN(currentStyledWidth) ||
      Math.abs(currentStyledWidth - expectedWidth) >= 0.5
    ) {
      targetElement.style.width = `${expectedWidth}px`;
    }

    // --- Height ---
    const expectedHeight = targetElement.offsetHeight * currentZoom;
    const currentWrapperHeight = parseFloat(wrapper.style.height);
    if (
      isNaN(currentWrapperHeight) ||
      Math.abs(currentWrapperHeight - expectedHeight) >= 0.5
    ) {
      wrapper.style.height = `${expectedHeight}px`;
    }

    // --- Callback ---
    this.settings.onDimensionsChange?.(currentZoom);
  };
  /**
